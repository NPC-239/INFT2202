...
const movie = {
    ...
    index: async function (_, res) {
        try {
            ...
            if (_.params.name) {
                ret = await movieService.query(_.params.name);
            } else {
                ret = await movieService.load(_.query);
            }
            res.json(ret);
        }
        catch (err) {
    ...
    add: async function (_, res) {
        try {
            ...
            let ret = await movieService.add(userData);
    ...
    update: async function (_, res) {
        try {
            ...
            let ret = await movieService.update(userData);
    ...
    delete: async function (_, res) {
        try {
            const user = _.headers['user'];
            if (_.params.name) {
                let ret = await movieService.delete(_.params.name);
                res.status(200).send({ message: ret });
            } else {
